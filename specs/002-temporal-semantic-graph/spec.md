...
- **FR-206**：系统必须基于真实数据和图结构计算一层更高阶的语义评分属性，而不是简单次数统计或虚构常量。至少包括：开发者节点级别的影响力评分（如 `influence_score`）、事件节点级别的重要性评分（如 `importance_score`）、仓库节点级别的活跃度评分（如 `activity_score`）、提交节点级别的重要性评分（如 `significance_score`）、开发者→事件边级别的贡献强度（如 `contribution_strength`）、事件→仓库边级别的影响评分（如 `impact_score`）、事件→提交边级别的关联强度（如 `relevance_score`）。这些评分需要可解释地依赖于事件类型、提交数量、跨仓库/跨时间分布、提交信息长度等因素，并采用**稳定的归一化方案**（例如 min-max 归一化，保证在 0～1 区间内且不丢失相对差异），便于在不同分钟、不同图之间进行比较与推理。
