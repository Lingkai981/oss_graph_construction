...
## 决策 9：其他节点与边的语义评分计算方式

- **结论**：
  - 仓库节点活跃度 `activity_score`：基于指向该仓库的所有事件的重要性得分总和，并考虑参与该仓库的不同开发者数量（对数加成），经 min-max 归一化映射到 [0,1]；
  - 提交节点重要性 `significance_score`：基于所属 PushEvent 的重要性得分，并考虑提交信息长度（可能反映重要程度），经 min-max 归一化映射到 [0,1]；
  - 事件→仓库边的 `impact_score`：直接使用事件的重要性评分，用于衡量该次行为对该仓库的影响强度；
  - 事件→提交边的 `relevance_score`：定义为“事件重要性评分 × 提交重要性评分”，用于衡量该提交在此次 PushEvent 中的相对重要性。
- **计算规则概述**：
  - 仓库原始活跃度：
    - 对每个仓库，累加所有指向该仓库的事件的重要性原始得分（`raw_imp`，已在决策8中计算）；
    - 统计参与该仓库的不同开发者数量 `actor_count`，并追加参与度加成 `participation_bonus = α * log1p(actor_count)`（当前实现中 `α` 为常量系数，例如 0.3）；
    - 得到 `repo_activity_raw = sum(raw_imp of events targeting this repo) + participation_bonus`。
  - 提交原始重要性：
    - 对每个提交，获取其所属 PushEvent 的重要性原始得分（`raw_imp`）；
    - 获取提交信息长度 `message_length`（字符数），并追加信息量因子 `message_factor = 1 + β * log1p(message_length)`（当前实现中 `β` 为常量系数，例如 0.1）；
    - 得到 `commit_significance_raw = raw_imp(push_event) * message_factor`。
    - **注意**：如果同一提交出现在多个 PushEvent 中（罕见但可能），则取最大值或累加值，具体实现时可根据业务需求选择。
  - 归一化：
    - 对仓库和提交分别使用 min-max 归一化，映射到 [0,1] 区间（与决策8中的归一化规则一致）。
  - 边级评分：
    - 对于事件→仓库边，`impact_score = importance_score(event)`（直接复用事件重要性，因为事件的重要性本身就反映了其对仓库的影响强度）；
    - 对于事件→提交边，`relevance_score = importance_score(event) * significance_score(commit)`（乘积形式，确保“重要事件中的重要提交”获得更高的关联强度）。
- **理由**：
  - 仓库活跃度通过累加事件重要性得分，可以捕捉“高价值事件越多，仓库越活跃”的直观语义；参与开发者数量的对数加成则体现了“多人协作活跃度更高”的特征，但通过对数缩放避免数量过多时线性放大。
  - 提交重要性基于所属 PushEvent 的重要性，符合“重要推送中的提交更可能重要”的直觉；提交信息长度作为补充因子，可以区分“有详细说明的提交”与“简单提交”，但影响相对较小（`β` 系数较小）。
  - 事件→仓库边的 `impact_score` 直接复用事件重要性，简化计算且语义清晰：事件越重要，对仓库的影响自然越大。
  - 事件→提交边的 `relevance_score` 使用乘积形式，确保“重要事件包含的重要提交”获得更高的关联评分，便于后续分析时按关联强度排序或过滤。
- **备选方案与后续优化方向**：
  - 可以在未来引入更复杂的图算法（如基于边权重的 PageRank）来计算仓库和提交的中心性得分，但当前特性优先采用简单、可解释且完全基于本文件数据的方案。
  - 参与度加成系数 `α`、信息量因子系数 `β` 可以在未来通过配置文件开放，允许用户根据研究需求调整权重。
  - 对于跨 PushEvent 的重复提交，当前实现可以选择取最大值（更保守）或累加值（更激进），具体选择应在实现文档中明确说明。
...
（6）在语义属性层面，除事件和开发者节点外，还为仓库节点、提交节点以及相关边引入语义评分（`activity_score`、`significance_score`、`impact_score`、`relevance_score`），形成完整的语义属性体系，所有评分均基于真实数据计算并经 min-max 归一化，确保可解释性和可比性。  
